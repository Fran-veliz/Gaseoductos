...
target_files = 'transformed_data.csv'
...
def extrac():
    extracted_data = pd.DataFrame(columns=["presion","diametro","tipogas","fechapuesta","estado","profundidad","codtramant","material","calificacion","propiedad","empresa","longitud","geometria_wkt"])
    
    for csvfile in glob.glob("*.csv"):
        if csvfile != target_files:
            data = extract_from_csv(csvfile)
            extracted_data = pd.concat([extracted_data, data], ignore_index=True)
            log("Data extraida correctamente de :"+ csvfile)
    return extracted_data

extracted_data = extrac()
'''
      Con esta funcion G¿generamos un perfil basico  con un 10 mil datos aleatorios para ver que tipo de errores nos escontramos 
      para su posterior creacion de funciones   para cada variable ,esto no se puede hacer de manera visual ya que son
      24 millones de datos 
    '''
...
# Duplicados exactos
dup_exactos = extracted_data.duplicated().sum()

# Duplicados por código
dup_por_codigo = extracted_data.duplicated(subset=['codtramant']).sum()

print(f"Total registros: {total}")
print(f"Duplicados exactos: {dup_exactos}")
print(f"Duplicados por código: {dup_por_codigo}")'''
...
def pipeline_limpieza(data):
    ...
    data[['lon_inicio', 'lat_inicio', 'lon_fin', 'lat_fin']] = data['geometria_wkt'].apply(cleanGeo)
    data = data.drop(columns=['geometria_wkt','fecha_corte','fecha_emision','codtramo'])

    return data

extracted_data = pipeline_limpieza(extracted_data)
extracted_data.to_csv("transformed_data.csv", index=False, encoding="utf-8")
